...
public abstract class InteractiveTileObject {
    ...
    public InteractiveTileObject(PlayScreen screen, Rectangle bounds) {
        ...
        bdef.position.set((bounds.getX() + bounds.getWidth() / 2) / QuickTiles.PPM, (bounds.getY() + bounds.getHeight() / 2) / QuickTiles.PPM);

        body = world.createBody(bdef);

        shape.setAsBox((bounds.getWidth() / 2) / QuickTiles.PPM, (bounds.getHeight() / 2) / QuickTiles.PPM);
        fdef.shape = shape;
    ...
    public TiledMapTileLayer.Cell getCell(int index) {
        ...
        return layer.getCell((int)(body.getPosition().x * QuickTiles.PPM / 16), (int)(body.getPosition().y * QuickTiles.PPM / 16));
    }
}
